...
def editKitList(oldItem):
	...
	for i in origKitList:
		...
		try:
			...
			amount = str(1)
		newName = inputY("Enter new name [" + str(name) + "] , or enter \'delete\' to remove ")
		if newName == "delete":
			continue
		elif not newName == "":
			name = newName
...
def getName(oldItem,greek):
	tempName = inputY("Name: [" + str(oldItem.name) + "] ")
	if not tempName == "":
		newName = tempName
...
while not doneEdits:
	...
	if selection == "edit" or selection == "new":
		if selection == "new":
