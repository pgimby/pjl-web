...
def printB(txt):
        printB(Fore.BLUE + txt + Style.RESET_ALL)
...
def getOriginalDir():
	...
	validDir = False
	while not validDir:
		originalDir = inputY("Enter absolute path for directory containing lab: ")
	...
	if inputY("Would you like to edit the equipment list for this lab? y/N ").lower() == "y":
		...
		while not allItems:
			...
			if inputY("Would you like to add a new piece of equipment for this lab? y/N ").lower() == "y":
				itemId = inputY("Enter the equipment id number: ")
...
def equipInfoReview(eqdb,originalItem):
	...
	if originalItem.equipment:
		for i in originalItem.equipment:
			...
			if input (Fore.YELLOW + "Would you like to edit this entry? y/N: ").lower() == "y":
				...
				if equipID == "delete":
					...
					if debug: printG("[Debug - equipInfoReview] editing equipment item???")
					item = addEquipItem(eqdb,equipID)
					if not item == "":
						if debug: printG("[Debug - equipInfoReview]  adding edited entry???")
...
def addEquipItem(eqdb,itemId):
	...
	while not validNum:
		...
		if amount.isdigit():
			...
			printB(amount + " is not a valid number.")
			if inputY("Do you wish to try again? Y/n: ").lower() == "n":
				return ""
		equipItem["id"]  = itemId
	return equipItem
...
def equipValid(eqdb,itemID):
	...
	'''
	if debug: printG("[Debug - equipValid]  entering")
	errorMessage = ""
...
#	print("[0] " + item.name)
	...
	while not validName:
		...
		if debug: printG("[Debug - equipNameOptions] selection " + str(selection))
...
def getSoftwareList(originalItem,softwareSource):
	...
	'''
	softwareItems = []
	if inputY("Would you like to edit the software list for this lab? y/N ").lower() == "y":
		print("")
		...
		allItems = False
		while not allItems:
			if inputY("Would you like to add a new software for this lab? y/N ").lower() == "y":
				print("")
...
def softwareRemove(originalItem):
	...
	if originalItem.software:
		for i in originalItem.software:
	 		if not inputY("Would you like to remove \"" + i + "\" as needed software? If so enter 'delete': ").lower() == "delete":
...
def getNewSoftware(masterList):
	...
	valid = False
	while not valid:
		item = inputY("Enter new software: ")
		for i in masterList:
			if i.lower() == item.lower():
		 		...
		 		print("")
		if not valid:
			printB(item + " is invalid software.")
...
def getDisciplineList(originalItem,disciplineSource):
	...
	'''
	disciplineItems = []
	if inputY("Would you like to edit the disciplines list for this lab? y/N ").lower() == "y":
		print("")
		...
		allItems = False
		while not allItems:
			if inputY("Would you like to add a new discipline for this lab? y/N ").lower() == "y":
				print("")
...
def disciplineRemove(originalItem):
	...
	if originalItem.disciplines:
		for i in originalItem.disciplines:
	 		if not inputY("Would you like to remove \"" + i + "\" as a discipline? If so enter 'delete': ").lower() == "delete":
	...
	while not valid:
		...
		for i in masterList:
			if i.lower() == item.lower():
		 		...
		 		print("")
		if not valid:
			printB(item + " is an invalid discipline.")
			if not inputY("Would you like to try again? Y/n ").lower() == "n":
	 			continue
			else:
				break
...
def getTopicList(originalItem,topicSource):
	...
	'''
	topicItems = []
	if inputY("Would you like to edit the topics list for this lab? y/N ").lower() == "y":
		print("")
		...
		allItems = False
		while not allItems:
			if inputY("Would you like to add a new topic for this lab? y/N ").lower() == "y":
				print("")
...
def topicRemove(originalItem):
	...
	if originalItem.topics:
		for i in originalItem.topics:
	 		if not inputY("Would you like to remove \"" + i + "\" as a topic? If so enter 'delete': ").lower() == "delete":
		 		topicItems.append(i)	 			
	return topicItems
	...
	while not valid:
		...
		for i in masterList:
			if i.replace("'","").lower() == item.replace("'","").lower():
		 		...
		 		print("")
		if not valid:
			printB(item + " is an invalid topic.")
			if not inputY("Would you like to try again? Y/n ").lower() == "n":
	 			continue
			else:
				break
...
def confirmEntry(new_version):
	...
	printY("Please confirm that the information entered is correct")
...
def getEditInfo(originalItem,eqdb,disciplineSource,topicSource,softwareSource,testMode):
	...
	'''
	if debug: printG("[Debug - getEditInfo]  entering")
...
def displayLabItem(lab):
	...
	if inputY("Is this information correct? N/y: ").lower() == "y":
		return True
	else:
		print("exiting...")
		return
...
#parser.add_argument('-e', dest="experiment", help="Configure a computer for specific experiment [0016, 0016demo, test]. Run with no argument complete list.", type=str, nargs='?', const="list")#'listValid')
...
#	newIdNum=labdb.new_id
#	print(newIdNum)
	lab = labdb.newLab(args.new)
