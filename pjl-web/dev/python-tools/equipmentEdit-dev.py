...
'''functions for committing of development version of xml, and backup of old version'''
def get_db_files():
...
def increment_files(files):
    ...
    os.system("cp " + new_xml + " " + db_storage + "equipmentDB.xml")
    os.system("chmod 644 " + db_storage + "equipmentDB.xml")
...
menu=[
	...
	{"option": "eqWheel", "name": "Wheel changes"}
	]
...
while not doneEdits:
	...
	'''wheel database'''
	if selection == "eqWheel":
		printB("Updating equipmentDB.xml in data folder.")    
		files = get_db_files()
