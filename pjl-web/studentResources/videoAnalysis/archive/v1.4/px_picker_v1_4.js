...
function videoLoaded(){  
  ...
  var fps=prompt("Please enter your video framerate (fps), or enter 'auto' to have the browser try to auto-detect it (recent Chrome browsers only).","30.0")  
  ...
  if (manualFramerateMode) {
	...
	console.log(navigator.userAgent.search("hrome/8"));
    if(navigator.userAgent.search("hrome/8") < 1)
      alert("Warning: your browser may struggle to auto-detect the framerate.");
    ...
    .then(fn =>{				
		...
		var txt=(1/videoDt).toFixed(2);
        alert("Auto-detected framerate is "+txt);
		videoSetup = true;
        controlsSetup();
        initCanvas();
