%Created on: Aug 15, 2014       Edited by: Wesley Kyle
%************************COMMENTS***********************************
% We discovered that the set up for this lab did not feed the 
% the oscilloscope accurate values for the voltages of the two
% signals. One signal followed a much more convoluted path through
% the resistors in the circuit so that by the time it was measured
% at the oscilloscope it was not only attenuated by the cable but
% also by the additional voltage drops due to the resistors in the
% reflectionless splitter. Under advisement from Pat Irwin we 
% changed the set up so that students could compare directly the 
% two signals (output and cable-attenuated) via channels 1 and 2.
% -Wesley Kyle (15 Aug, 2014)
%*******************************************************************
%Edited on: Sept 8, 2015        Edited by: Peter Gimby - added companion guide
...
%Edited on: Aug 22, 2017        Edited by: Ania Harlick
%Edited on: Sep 5, 2017		    Edited by: Peter Gimby - fixed a table/label overlap
...
\node[scale=1.23,opacity=.6]at(5.4,4){\includegraphics{pulse-low-res.jpg}};
...
\begin{tikzpicture}
%\draw[very thin](0,0)rectangle(11,8);
\node[scale=1.2,opacity=.6]at(5.6,4){\includegraphics{pulse-high-res.jpg}};
...
\begin{tikzpicture}
%\draw[very thin](0,0)rectangle(11,8);
\node[scale=1.2,opacity=.8]at(5.5,4){\includegraphics{open-end-reflection.jpg}};
...
\begin{tikzpicture}
%\draw[very thin](0,0)rectangle(11,8);
\node[scale=1.2,opacity=.9]at(5.5,4){\includegraphics{shorted-reflection.jpg}};
...
\begin{marginfigure}
\begin{tikzpicture}
\node[above right,scale=1.15]at(0,0){\includegraphics{superimposed-reflection.jpg}};
%1st plateau
...
\begin{marginfigure}
\begin{tikzpicture}
\node[above right,scale=1.1]at(0,0){\includegraphics{rg59-open.jpg}};
\node[]at(1,4.3){A};
\node[]at(1.8,1.4){B};
\node[]at(2.6,2.4){C};
\node[]at(3.4,1){D};
...
\begin{marginfigure}
\begin{tikzpicture}
\node[above right,scale=1.1]at(0,0){\includegraphics{coax-shorted.jpg}};
\node[]at(1,4.3){A};
\node[]at(1.8,2.2){B};
\node[]at(2.6,1){C};
\node[]at(3.3,1){D};
