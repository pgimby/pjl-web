<?php
// Cross-Origin Resource Sharing (CORS) must be allowed if the php script is accessed from a different domain.
// Very useful for testing, but could be commented out for production code.
header("Access-Control-Allow-Origin: *");

// Lab computer URL (eg. http://franklin.pjl.ucalgary.ca:4444 )
$lab_URL = 'http://mole.pjl.ucalgary.ca:4444';
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
  ...
  } else if(isset($_POST['magData']))
  {
    // Get the requested time
    $lastTime=$_POST['magData'];

    // Prepare POST request for lab computer
    $data = array('magData' => $lastTime);
    $options = array('http' => array(
                     'header'  => "Content-type: application/x-www-form-urlencoded\r\n",
                     'method'  => 'POST',
                       'content' => http_build_query($data)));
